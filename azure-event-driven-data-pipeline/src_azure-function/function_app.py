...
# Cosmos DB client setup - make sure to set COSMOSDB_URL and COSMOSDB_KEY in your function app settings



@app.event_hub_message_trigger(arg_name="azeventhub", 
                               event_hub_name="azu2602-ehubs", 
                               connection="EventHubConnection",
                               consumer_group="cg-cosmosdb")
def eventhub_to_cosmos(azeventhub: func.EventHubEvent):
    conn_str = os.getenv("COSMOSDB_CONNECTION_STRING")
    db_name = "click-streams-db"
    container_name = "events"
    
    try:
        # 2. Initialize the client ONLY if we have a connection string
        if not conn_str:
            logging.error("COSMOSDB_CONNECTION_STRING is missing from App Settings!")
            return
        ...
        # 3. Process the Event Hub message
        message_body = azeventhub.get_body().decode("utf-8")
        data = json.loads(message_body)

        # Map 'eventId' to 'id' for Cosmos requirement
        data['id'] = data.get('eventId')

        # 4. Perform the Upsert
        cosmos_container.upsert_item(data)
        logging.info(f"✅ Successfully indexed event {data['id']}")

    except Exception as e:
        logging.error(f"❌ Error in eventhub_to_cosmos: {str(e)}")

@app.cosmos_db_trigger(
    arg_name="documents",
    database_name="click-streams-db",
    container_name="events",
...
def cosmos_trigger_counter(documents: func.DocumentList, outputDocument: func.Out[func.Document]):
    for doc in documents:
        user_id = doc.get("userId")
        event_tye = doc.get("eventType")

        # 1. Logic: Prepare the state uodate
        # we treat this as a 'Partial Updates' (Upsert)

        user_profile_update = {
            "id": user_id,  # Partition key
            "userId": user_id,
            "lastEventType": event_tye,
            "lastActive": doc.get("timestamp"),
            "isPopupEligible": True if event_tye == "cart_abandon" else False
        }

        logging.info(f"Updating Global Counter for User: {user_id}")

        outputDocument.set(func.Document.from_dict(user_profile_update))